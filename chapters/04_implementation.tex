...
\textbf{État d'avancement.} L'ensemble de ce module est réalisé et
vérifié, y compris la console web activable ou désactivable
indépendamment par serveur ou par conteneur.
...
fichier détaillé sur disque, diffusé simultanément en direct sur un
...
\textbf{État d'avancement.} Le socle du module est réalisé et vérifié
en conditions réelles. Restent non commencées : la mise en place d'un
délai d'expiration propre à chaque phase, la déduplication des
évènements de webhook reçus en double, et l'annulation coopérative d'un
déploiement en cours d'exécution.
...
\textbf{Configuration du proxy.} La configuration du serveur web et la
demande de certificat HTTPS sont générées puis appliquées sur le
...
\textbf{État d'avancement.} Le module est considéré comme
fonctionnellement clos. Deux points restent non implémentés :
la dégradation gracieuse vers un mode HTTP seul en cas d'échec de
l'émission du certificat, et le filtrage par ressource du contrôle
d'accès sur les routes indexées directement par une publication (ces
routes restent protégées par une permission globale, faute d'une portée
dédiée à ce type de ressource dans le modèle RBAC/IBAC).
...
\textbf{État d'avancement.} Le module a été vérifié en conditions
réelles jusqu'au déclenchement puis à la résolution automatique d'une
alerte. Une limite connue subsiste : le filtrage des métriques liées à
Docker capture l'ensemble des conteneurs présents sur l'hôte, pas
seulement ceux gérés par AGT Infra.

\subsection{Contrôle d'accès dynamique (RBAC/IBAC)}

\textbf{Modèle de données.} Les rôles et les permissions ne sont plus
...
si aucune de ces conditions n'est remplie. Cette vérification est
appliquée de façon uniforme sur les routes classiques comme sur les
...
\textbf{Correctif de sécurité.} La révocation de l'affectation du rôle
immuable a été bloquée après qu'un scénario de test a révélé qu'elle
pouvait conduire à un verrouillage total de l'application si plus aucun
compte ne disposait de ce rôle. Cette opération est désormais rejetée
côté serveur et masquée dans l'interface.

\textbf{État d'avancement.} Le socle est réalisé et vérifié. Le
filtrage par ressource reste à généraliser aux publications, aux
alertes et aux sessions de terminal, encore protégées par une
permission globale à ce jour.
